...
<div data-options="region:'north',border:false" style="height:60px;background:#B3DFDA;padding:10px">
    ...
    <div style="float:right;margin-top:10px">
        ...
        <div id="modify_info_dialog" class="easyui-dialog" closed="true"   buttons="#dlg-buttons" style="width:400px;height:298px;padding-left:30px;padding-right:30px;padding-top:10px">
            <form id="private_modify_form" method="post" action="${pageContext.request.contextPath}/modifyAdmin.action">
                <div style="margin-bottom:20px">
                    <input id="form-username" class="easyui-textbox" name="username" label="用户名" style="width:75%;" data-options="disabled:true">
                    <input id="username_hidden" type="hidden" name="username">
                </div>
                <div style="margin-bottom:20px">
                    <input id="form-name" class="easyui-textbox" label="姓名" name="name" style="width:75%;">
                </div>
                <div style="margin-bottom:20px">
                    <input id="form-oldpassword" class="easyui-passwordbox" label="原密码" prompt="旧密码" iconWidth="28" style="width:75%;">
                </div>
                <div style="margin-bottom:20px">
                    <input id="form-newpassword" class="easyui-passwordbox"  label="新密码" prompt="新密码" iconWidth="28" style="width:75%;">
                </div>
                <div style="margin-bottom:20px">
                    <input id="form-repeatpassword" name="password" class="easyui-passwordbox" label="重复密码" prompt="重复新密码" iconWidth="28" validType="confirmPass['#form-newpassword']" style="width:75%;">
                </div>
            </form>
            <div id="dlg-buttons">
...
<script>
    ...
    function openInfoDialog(){
        ...
        $("#form-username").textbox('setValue',loginuser.username);
        $("#form-name").textbox('setValue',loginuser.name);
    ...
    function submitForm(){
        $('#private_modify_form').form('submit',{
            onSubmit:function(){
                if (oldPassword !=  $('#form-oldpassword').passwordbox('getValue')) {
                    alert("原密码错误");
                    clearForm();
                    return false;
                }
                if ($('#form-newpassword').passwordbox('getValue') != $('#form-repeatpassword').passwordbox('getValue')) {
                    alert("两次密码不一致!");
                    clearForm();
                    return false;
                }
                newName = $('#form-name').textbox('getValue');
                newPassword = $('#form-repeatpassword').textbox('getValue');
                return true;
            },
            success:function(data){
                ...
                if (dataobj.msg == "success") {
                    //更新loginuser对象
                    loginuser = {"username":"${sessionScope.admin.username}","name":newName,"password":newPassword};
                    oldPassword = newPassword;
                    //更新右上角名字
                    $("#login_user").text(newName);
    ...
    function clearForm(){
        $('#form-oldpassword').passwordbox('clear');
        $('#form-newpassword').passwordbox('clear');
        $('#form-repeatpassword').passwordbox('clear');
    }
    //退出系统
    function exitSystem(){
